...
async function populateDepartmentCode() {
  let connection;
  ...
  try {
    ...
    // 1. 获取部门映射关系 (department_name -> department_code) from departments table
    console.log('\n📊 获取部门映射关系...');
    ...
    // 2. 更新assets表的asset_department_code字段
    console.log('\n🔄 更新assets表的asset_department_code字段...');

    // 首先获取所有需要更新的资产
    const [assets] = await connection.execute(`
      SELECT id, department FROM assets WHERE department IS NOT NULL AND department != '' AND (asset_department_code IS NULL OR asset_department_code = '')
    `);
