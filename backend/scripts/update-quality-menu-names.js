/**
 * 更新质量管理菜单名称
 * 一级菜单：质量控制 -> 质量管理
 * 二级菜单：添加"质量控制"菜单项
 */

require('dotenv').config();
const mysql = require('mysql2/promise');
const { database: databaseConfig } = require('../config/app.config');

async function updateMenuNames() {
  let connection;
  try {
    console.log('开始更新菜单名称...\n');

    // 创建数据库连接
    connection = await mysql.createConnection({
      host: databaseConfig.host,
      port: databaseConfig.port,
      user: databaseConfig.user,
      password: databaseConfig.password,
      database: databaseConfig.database,
      connectTimeout: 10000,
    });

    console.log('✓ 数据库连接成功\n');

    // 1. 确保一级菜单名称为"质量管理"
    await connection.execute('UPDATE menu_definitions SET menu_label = ? WHERE menu_key = ?', [
      '质量管理',
      '/quality-control-parent',
    ]);
    console.log('✓ 一级菜单已更新为"质量管理"\n');

    // 2. 添加"质量控制"二级菜单（如果不存在）
    await connection.execute(
      `
      INSERT INTO menu_definitions (menu_key, menu_label, parent_key, icon, order_index, is_active)
      VALUES (?, ?, ?, ?, ?, 1)
      ON DUPLICATE KEY UPDATE 
        menu_label = VALUES(menu_label),
        parent_key = VALUES(parent_key),
        order_index = VALUES(order_index),
        updated_at = CURRENT_TIMESTAMP
    `,
      ['/quality-control/quality-control', '质量控制', '/quality-control-parent', null, 1],
    );
    console.log('✓ 已添加/更新二级菜单"质量控制"\n');

    // 3. 更新其他二级菜单的排序
    await connection.execute('UPDATE menu_definitions SET order_index = ? WHERE menu_key = ?', [
      2,
      '/quality-control/metrology',
    ]);
    console.log('✓ 计量管理菜单排序已更新为 2\n');

    await connection.execute('UPDATE menu_definitions SET order_index = ? WHERE menu_key = ?', [
      3,
      '/quality-control/qc',
    ]);
    console.log('✓ 质控管理菜单排序已更新为 3\n');

    // 4. 为所有角色设置新菜单的权限
    const [roles] = await connection.execute('SELECT role_code FROM roles');
    const [menus] = await connection.execute(
      'SELECT menu_key FROM menu_definitions WHERE menu_key LIKE "/quality-control%"',
    );

    console.log('正在更新菜单权限...\n');
    for (const role of roles) {
      const roleCode = role.role_code;
      for (const menu of menus) {
        const isVisible = roleCode === 'system_admin' ? 1 : 0;
        await connection.execute(
          `
          INSERT INTO role_menu_permissions (role, menu_key, is_visible)
          VALUES (?, ?, ?)
          ON DUPLICATE KEY UPDATE is_visible = VALUES(is_visible)
        `,
          [roleCode, menu.menu_key, isVisible],
        );
      }
    }
    console.log('✓ 菜单权限已更新\n');

    // 5. 显示更新后的菜单结构
    const [updatedMenus] = await connection.execute(`
      SELECT menu_key, menu_label, parent_key, order_index
      FROM menu_definitions
      WHERE menu_key LIKE '/quality-control%'
      ORDER BY order_index, id
    `);

    console.log('更新后的菜单结构:');
    console.log('─────────────────────────────────────');
    updatedMenus.forEach(menu => {
      const indent = menu.parent_key ? '  └─ ' : '';
      console.log(`${indent}${menu.menu_label} (${menu.menu_key}) [排序: ${menu.order_index}]`);
    });
    console.log('─────────────────────────────────────\n');

    await connection.end();
    console.log('✅ 菜单更新完成！');
    return true;
  } catch (error) {
    console.error('\n❌ 更新失败:');
    console.error('错误代码:', error.code);
    console.error('错误消息:', error.message);
    if (connection) {
      try {
        await connection.end();
      } catch (e) {
        // 忽略关闭错误
      }
    }
    return false;
  }
}

// 运行更新
updateMenuNames()
  .then(success => {
    process.exit(success ? 0 : 1);
  })
  .catch(error => {
    console.error('脚本执行异常:', error);
    process.exit(1);
  });
