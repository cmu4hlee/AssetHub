...
CREATE TABLE IF NOT EXISTS metrology_records (
  ...
  asset_id INT NOT NULL DEFAULT 0 COMMENT '资产ID（兼容字段）',
  ...
  technical_document VARCHAR(500) COMMENT '技术文件',
  conformance_standard VARCHAR(200) COMMENT '依据标准',
  ...
  INDEX idx_tenant_asset (tenant_id, asset_code),
...
CREATE TABLE IF NOT EXISTS metrology_attachments (
  ...
  created_by VARCHAR(50) COMMENT '上传人',
...
CREATE TABLE IF NOT EXISTS quality_control_records (
  ...
  tolerance VARCHAR(100) COMMENT '允许偏差',
  result ENUM('合格', '不合格', '待检', '整改中') DEFAULT '待检' COMMENT '质控结果',
  qc_person VARCHAR(50) COMMENT '质控人',
...
CREATE TABLE IF NOT EXISTS quality_management_alerts (
  ...
  INDEX idx_tenant_unhandled (tenant_id, is_handled)
...
SELECT 'metrology_records' as table_name, COUNT(*) as exists_check FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'metrology_records'
UNION ALL
SELECT 'metrology_attachments', COUNT(*) FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'metrology_attachments'
UNION ALL
SELECT 'quality_control_records', COUNT(*) FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'quality_control_records'
UNION ALL
SELECT 'quality_control_attachments', COUNT(*) FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'quality_control_attachments'
UNION ALL
SELECT 'quality_management_alerts', COUNT(*) FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'quality_management_alerts'
UNION ALL
SELECT 'quality_management_cycles', COUNT(*) FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'quality_management_cycles'
UNION ALL
SELECT 'metrology_record_sequence', COUNT(*) FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'metrology_record_sequence'
UNION ALL
SELECT 'qc_record_sequence', COUNT(*) FROM information_schema.tables
WHERE table_schema = 'zcgl' AND table_name = 'qc_record_sequence';

-- ============================================
-- 脚本执行完成提示
-- ============================================
-- 如果所有表都显示 exists_check = 1，说明表创建成功
-- 如果显示 0，说明表创建失败，请检查错误信息
