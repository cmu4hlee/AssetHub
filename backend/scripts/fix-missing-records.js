...
async function fixMissingRecords() {
  ...
  try {
    ...
    // 缺失记录的行号（从1开始计数）
    const missingRows = [9743, 21859, 22621]; // 注意：数组索引从0开始，所以行号减1
    ...
    for (const rowIndex of missingRows) {
      ...
      try {
        ...
        const assetCode =
          originalCode3 || originalCode || originalCode2 || `ZC${Date.now()}${rowIndex}`;

        console.log(`  资产名称: ${row['name']}`);
        console.log(`  资产编号: ${assetCode}`);
        console.log(`  原始编码: ${originalCode}, ${originalCode2}, ${originalCode3}`);

        // 处理价格
        const priceStr = (row['price'] || '0').toString().replace(/[,\t]/g, '').trim();
        const price = parseFloat(priceStr) || 0;

        const assetName = (row['name'] || '').toString().trim() || `资产${rowIndex + 1}`;
        ...
        const [existing] = await connection.execute(
          'SELECT id FROM assets WHERE asset.code = ? OR code = ? OR code2 = ? OR code3 = ?',
          [assetData.asset.code, assetData.code, assetData.code2, assetData.code3],
        );
