...
async function initQualityManagementTables() {
  console.log('[质量管理] 开始初始化数据库表...');
  ...
  try {
    ...
    console.log('[质量管理] ✓ 数据库连接正常');

    // 创建计量管理表
    try {
      console.log('[质量管理] 正在创建表: metrology_records');
      ...
      `);
      console.log('[质量管理] ✓ metrology_records 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ metrology_records 表创建失败:', err.message);
      throw err;
    }

    // 创建计量附件表
    try {
      console.log('[质量管理] 正在创建表: metrology_attachments');
      await connection.execute(`
      ...
      `);
      console.log('[质量管理] ✓ metrology_attachments 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ metrology_attachments 表创建失败:', err.message);
      throw err;
    }

    // 创建质控管理表
    try {
      console.log('[质量管理] 正在创建表: quality_control_records');
      await connection.execute(`
      ...
      `);
      console.log('[质量管理] ✓ quality_control_records 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ quality_control_records 表创建失败:', err.message);
      throw err;
    }

    // 创建质控附件表
    try {
      console.log('[质量管理] 正在创建表: quality_control_attachments');
      await connection.execute(`
      ...
      `);
      console.log('[质量管理] ✓ quality_control_attachments 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ quality_control_attachments 表创建失败:', err.message);
      throw err;
    }

    // 创建质量管理预警记录表
    try {
      console.log('[质量管理] 正在创建表: quality_management_alerts');
      await connection.execute(`
      ...
      `);
      console.log('[质量管理] ✓ quality_management_alerts 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ quality_management_alerts 表创建失败:', err.message);
      throw err;
    }

    // 创建质量管理周期配置表
    try {
      console.log('[质量管理] 正在创建表: quality_management_cycles');
      await connection.execute(`
      ...
      `);
      console.log('[质量管理] ✓ quality_management_cycles 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ quality_management_cycles 表创建失败:', err.message);
      throw err;
    }

    // 创建计量单号序列表（修复 CLAUDE.md 中提到的序列缺失）
    try {
      console.log('[质量管理] 正在创建表: metrology_record_sequence');
      await connection.execute(`
      ...
      `);
      console.log('[质量管理] ✓ metrology_record_sequence 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ metrology_record_sequence 表创建失败:', err.message);
      throw err;
    }

    // 创建质控单号序列表（同上修复）
    try {
      console.log('[质量管理] 正在创建表: qc_record_sequence');
      await connection.execute(`
      ...
      `);
      console.log('[质量管理] ✓ qc_record_sequence 表创建成功');
    } catch (err) {
      console.error('[质量管理] ✗ qc_record_sequence 表创建失败:', err.message);
      throw err;
    }

    console.log('[质量管理] ✅ 所有表初始化完成！');

    // 关闭连接
    await connection.end();
    return true;
  } catch (error) {
    console.error('[质量管理] ❌ 表初始化失败:');
...
if (require.main === module) {
  initQualityManagementTables()
    .then(success => {
      if (success) {
        console.log('[质量管理] 初始化脚本执行成功');
        process.exit(0);
      } else {
        console.error('[质量管理] 初始化脚本执行失败');
        process.exit(1);
      }
    })
    .catch(error => {
      console.error('[质量管理] 初始化脚本执行异常:', error);
      process.exit(1);
    });
}

module.exports = initQualityManagementTables;
