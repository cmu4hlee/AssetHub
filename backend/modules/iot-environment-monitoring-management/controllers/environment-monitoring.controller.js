const environmentMonitoringPipelineService = require('../services/environment-monitoring-pipeline.service');
...
class EnvironmentMonitoringController {
  async ingest(req, res) {
    try {
      ...
      if (Array.isArray(req.body)) {
        ...
        const batchResult = await environmentMonitoringPipelineService.ingestBatch(events, 'http_batch', {
          request_ip: req.ip,
      ...
      const result = await environmentMonitoringPipelineService.ingestEvent(req.body || {}, 'http', {
        request_ip: req.ip,
        iot_auth_tenant_id: req.iotAuth?.tenant_id || null,
      });
  ...
  async ingestSample(req, res) {
    try {
      ...
      const batchResult = await environmentMonitoringPipelineService.ingestBatch(events, 'ui_sample', {
        request_ip: req.ip,
  ...
  async getLatestByDevice(req, res) {
    try {
      ...
      const data = await environmentMonitoringPipelineService.getLatestByDevice(deviceId, tenantId);
      if (!data) {
  ...
  async getAssetSeries(req, res) {
    try {
      ...
      const data = await environmentMonitoringPipelineService.getAssetSeries(assetCode, tenantId, req.query || {});
      res.json({ success: true, data });
    } catch (error) {
  ...
  async getLatestByAsset(req, res) {
    try {
      ...
      const data = await environmentMonitoringPipelineService.getLatestByAsset(assetCode, tenantId);
      if (!data) {
  ...
  async pipelineHealth(req, res) {
    try {
      await environmentMonitoringPipelineService.init();
      res.json({ success: true, data: environmentMonitoringPipelineService.getPipelineHealth() });
    } catch (error) {
  ...
  async pipelineDocs(req, res) {
    try {
      await environmentMonitoringPipelineService.init();
      res.json({ success: true, data: environmentMonitoringPipelineService.getPipelineDocs() });
    } catch (error) {
