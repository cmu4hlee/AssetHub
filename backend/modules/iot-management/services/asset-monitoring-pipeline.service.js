...
class AssetMonitoringPipelineService {
  constructor() {
  ...
  async init() {
    ...
    try {
      ...
      console.error('[AssetMonitoring] 时序表初始化失败:', error.message);
    }
    try {
      await this.initKafkaProducer();
    } catch (error) {
      console.warn('[AssetMonitoring] Kafka 初始化跳过:', error.message);
    }
    try {
      await this.initMqttSubscriber();
    } catch (error) {
      console.warn('[AssetMonitoring] MQTT 初始化跳过:', error.message);
    }
  ...
  async initKafkaProducer() {
    if (process.env.IOT_ASSET_MON_KAFKA_ENABLED !== 'true') return;
    try {
      const { Kafka } = require('kafkajs');
      const brokers = (process.env.IOT_ASSET_MON_KAFKA_BROKERS || '127.0.0.1:9092')
        .split(',')
        .map(item => item.trim())
        .filter(Boolean);
      this.kafkaClient = new Kafka({
        clientId: process.env.IOT_ASSET_MON_KAFKA_CLIENT_ID || 'assettube-asset-monitoring',
        brokers,
      });
      this.kafkaProducer = this.kafkaClient.producer();
      await this.kafkaProducer.connect();
      console.log('[AssetMonitoring] Kafka producer connected:', brokers.join(','));
    } catch (error) {
      console.warn('[AssetMonitoring] Kafka init skipped:', error.message);
      this.kafkaProducer = null;
    }
  }

  async initMqttSubscriber() {
    if (process.env.IOT_ASSET_MON_MQTT_ENABLED !== 'true') return;
    try {
      const mqtt = require('mqtt');
      const brokerUrl = process.env.IOT_ASSET_MON_MQTT_BROKER_URL || 'mqtt://127.0.0.1:1883';
      const topic = process.env.IOT_ASSET_MON_MQTT_TOPIC || 'iot/asset-monitoring/+/up';
      this.mqttClient = mqtt.connect(brokerUrl, {
        username: process.env.IOT_ASSET_MON_MQTT_USERNAME || undefined,
        password: process.env.IOT_ASSET_MON_MQTT_PASSWORD || undefined,
      });

      this.mqttClient.on('connect', () => {
        this.mqttClient.subscribe(topic, err => {
          if (err) {
            console.error('[AssetMonitoring] MQTT subscribe failed:', err.message);
            return;
          }
          console.log('[AssetMonitoring] MQTT subscribed:', topic);
        });
      });
      ...
      this.mqttClient.on('message', async (receivedTopic, messageBuffer) => {
        try {
          ...
          console.error('[AssetMonitoring] MQTT message process failed:', error.message);
        }
      });

      this.mqttClient.on('error', error => {
        console.error('[AssetMonitoring] MQTT client error:', error.message);
      });
    } catch (error) {
      console.warn('[AssetMonitoring] MQTT init skipped:', error.message);
      this.mqttClient = null;
    }
  }
  ...
  normalizePayload(rawPayload = {}) {
    ...
    return {
      ...
      runtime_state: normalizeString(merged.runtime_state || merged.runtimeState),
      signal_strength: merged.signal_strength ?? merged.signalStrength,
      battery_level: merged.battery_level ?? merged.batteryLevel,
      cpu_usage: merged.cpu_usage ?? merged.cpuUsage,
      memory_usage: merged.memory_usage ?? merged.memoryUsage,
      error_code: normalizeString(merged.error_code || merged.errorCode),
  ...
  normalizeErrorReportRow(row) {
    const payload = this.parsePayloadJson(row.payload_json);

    return {
      ...row,
      error_message: payload?.error_message || payload?.errorMessage || null,
      error_analysis: payload?.error_analysis || payload?.errorAnalysis || null,
      severity: payload?.severity || null,
      screenshot_urls: Array.isArray(payload?.screenshot_urls) ? payload.screenshot_urls : [],
    ...
    for (let i = 0; i < rows.length; i += chunkSize) {
      ...
      await db.execute(
        `INSERT INTO iot_asset_monitor_ts (
          ...
          runtime_state, signal_strength, battery_level,
          cpu_usage, memory_usage, error_code,
          event_time, ingest_source, payload_json
        ) VALUES ${placeholders}`,
        values,
      );
    }
  ...
  async publishKafkaEventsBatch(eventPayloads) {
    ...
    const topic = process.env.IOT_ASSET_MON_KAFKA_TOPIC || 'iot.asset.monitoring.raw';
    ...
    try {
      ...
      console.error('[AssetMonitoring] Kafka batch publish failed:', error.message);
      return false;
    }
  }
  ...
  async ingestEvent(rawPayload, source = 'http', meta = {}) {
    ...
    if (batchResult.success !== 1 || !batchResult.results[0]?.success) {
      throw new Error(batchResult.results[0]?.message || '资产监测数据接收失败');
    }
  ...
  async ingestBatch(rawEvents = [], source = 'http_batch', meta = {}) {
    ...
    normalizedEvents.forEach(payload => {
      ...
      const hasMonitoringMetric =
        hasMeaningfulMetricValue(payload.runtime_state) ||
        hasMeaningfulMetricValue(payload.signal_strength) ||
        hasMeaningfulMetricValue(payload.battery_level) ||
        hasMeaningfulMetricValue(payload.cpu_usage) ||
        hasMeaningfulMetricValue(payload.memory_usage) ||
        hasMeaningfulMetricValue(payload.error_code);

      if (!hasMonitoringMetric) {
        results.push({ success: false, message: '监测参数不能为空' });
        return;
      }
      ...
      insertRows.push([
        ...
        this.parseIntSafe(payload.signal_strength),
        this.parseIntSafe(payload.battery_level),
        this.parseFloatSafe(payload.cpu_usage),
        this.parseFloatSafe(payload.memory_usage),
  ...
  async publishKafkaEvent(eventPayload) {
    ...
    try {
      await this.kafkaProducer.send({
        topic,
        messages: [{ key: eventPayload.device_id, value: JSON.stringify(eventPayload) }],
      });
      return true;
    } catch (error) {
      console.error('[AssetMonitoring] Kafka publish failed:', error.message);
      return false;
    }
  }
  ...
  async getLatestByDevice(deviceId, tenantId) {
    const [rows] = await db.execute(
      `SELECT * FROM iot_asset_monitor_ts
  ...
  async getErrorReportsByAsset(assetCode, tenantId, query = {}) {
    const page = Math.max(parseInt(query.page || 1, 10), 1);
    const pageSize = Math.min(
      Math.max(parseInt(query.pageSize || query.page_size || query.limit || 20, 10), 1),
      100,
    );
    const offset = (page - 1) * pageSize;

    const params = [tenantId, assetCode, 'error', 'external_app_report'];
    let whereClause = `
      WHERE tenant_id = ?
        AND asset_code = ?
        AND (
    ...
    return {
      asset_code: assetCode,
      list: rows.map(row => this.normalizeErrorReportRow(row)),
      pagination: {
        page,
        page_size: pageSize,
        total,
  ...
  getPipelineHealth() {
    return {
      mqtt_enabled: process.env.IOT_ASSET_MON_MQTT_ENABLED === 'true',
      mqtt_connected: !!this.mqttClient?.connected,
      kafka_enabled: process.env.IOT_ASSET_MON_KAFKA_ENABLED === 'true',
      kafka_connected: !!this.kafkaProducer,
      tsdb: process.env.IOT_ASSET_MON_TSDB || 'mysql_ts_table',
  ...
  getPipelineDocs() {
    return {
      ...
      http_ingest_path: '/api/iot/asset-monitoring/ingest',
      http_ingest_batch_path: '/api/iot/asset-monitoring/ingest/batch',
      external_report_path: '/api/iot/asset-monitoring/external-report',
      asset_error_reports_path: '/api/iot/asset-monitoring/assets/:assetCode/error-reports',
      auth: {
        token_header: 'x-iot-token',
        bearer_header: 'Authorization: Bearer <token>',
        query_token: 'token',
        external_report_headers: {
          authorization: 'Bearer <jwt>',
          tenant_header: 'X-Tenant-ID: <tenantId> (optional, multi-tenant switch)',
        },
      },
      mqtt: {
        enabled: process.env.IOT_ASSET_MON_MQTT_ENABLED === 'true',
        topic: process.env.IOT_ASSET_MON_MQTT_TOPIC || 'iot/asset-monitoring/+/up',
      },
      kafka: {
        enabled: process.env.IOT_ASSET_MON_KAFKA_ENABLED === 'true',
        topic: process.env.IOT_ASSET_MON_KAFKA_TOPIC || 'iot.asset.monitoring.raw',
      },
      payload_examples: {
        flat: {
          device_id: 'MON001',
        ...
        error_report_query: {
          path: '/api/iot/asset-monitoring/assets/ASSET-DEMO-001/error-reports?page=1&pageSize=20',
          headers: {
            authorization: 'Bearer <jwt>',
          },
        },
...
module.exports = new AssetMonitoringPipelineService();
