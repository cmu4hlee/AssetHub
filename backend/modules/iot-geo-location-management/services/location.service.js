...
class GeoLocationService {
  ...
  async initGeoPipeline() {
    if (this.initialized) {
      return;
    }
    try {
      await this.ensureGeoTimeSeriesTable();
    } catch (error) {
      console.error('[GeoLocation] 时序表初始化失败:', error.message);
    }
    try {
      await this.initKafkaProducer();
    } catch (error) {
      console.warn('[GeoLocation] Kafka 初始化跳过:', error.message);
    }
    try {
      await this.initMqttSubscriber();
    } catch (error) {
      console.warn('[GeoLocation] MQTT 初始化跳过:', error.message);
    }
    this.initialized = true;
  }

  async ensureGeoTimeSeriesTable() {
    await db.execute(`
      CREATE TABLE IF NOT EXISTS iot_geo_location_ts (
        ...
        INDEX idx_geo_ts_tenant_device_time (tenant_id, device_id, event_time),
        INDEX idx_geo_ts_asset_time (asset_code, event_time),
        INDEX idx_geo_ts_event_time (event_time)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='地理定位时序数据表';
    `);
  }

  async initKafkaProducer() {
    if (process.env.IOT_GEO_KAFKA_ENABLED !== 'true') {
      return;
    }
    try {
      const { Kafka } = require('kafkajs');
      const brokers = (process.env.IOT_GEO_KAFKA_BROKERS || '127.0.0.1:9092')
        .split(',')
        .map(item => item.trim())
        .filter(Boolean);

      this.kafkaClient = new Kafka({
        clientId: process.env.IOT_GEO_KAFKA_CLIENT_ID || 'assettube-geo-location',
        brokers,
      });

      this.kafkaProducer = this.kafkaClient.producer();
      await this.kafkaProducer.connect();
      console.log('[GeoLocation] Kafka producer connected:', brokers.join(','));
    } catch (error) {
      console.warn('[GeoLocation] Kafka init skipped:', error.message);
      this.kafkaProducer = null;
    }
  }

  async initMqttSubscriber() {
    if (process.env.IOT_GEO_MQTT_ENABLED !== 'true') {
      return;
    }
    try {
      const mqtt = require('mqtt');
      const brokerUrl = process.env.IOT_GEO_MQTT_BROKER_URL || 'mqtt://127.0.0.1:1883';
      const topic = process.env.IOT_GEO_MQTT_TOPIC || 'iot/geo/+/up';

      this.mqttClient = mqtt.connect(brokerUrl, {
        username: process.env.IOT_GEO_MQTT_USERNAME || undefined,
        password: process.env.IOT_GEO_MQTT_PASSWORD || undefined,
      });

      this.mqttClient.on('connect', () => {
        this.mqttClient.subscribe(topic, err => {
          if (err) {
            console.error('[GeoLocation] MQTT subscribe failed:', err.message);
            return;
          }
          console.log('[GeoLocation] MQTT subscribed:', topic);
        });
      });
      ...
      this.mqttClient.on('message', async (receivedTopic, messageBuffer) => {
        try {
          ...
          await this.ingestGeoEvent(payload, 'mqtt', { mqtt_topic: receivedTopic });
        } catch (error) {
          console.error('[GeoLocation] MQTT message process failed:', error.message);
        }
      });

      this.mqttClient.on('error', error => {
        console.error('[GeoLocation] MQTT client error:', error.message);
      });
    } catch (error) {
      console.warn('[GeoLocation] MQTT init skipped:', error.message);
      this.mqttClient = null;
    }
  }
  ...
  normalizePayload(rawPayload = {}) {
    ...
    return {
      ...
      location_type: normalizeString(merged.location_type || merged.locationType),
      address: normalizeString(merged.address),
  ...
  async insertBatchRows(rows) {
    ...
    for (let i = 0; i < rows.length; i += chunkSize) {
      ...
      await db.execute(
        `INSERT INTO iot_geo_location_ts (
          tenant_id, device_id, asset_code,
          latitude, longitude, altitude, speed, heading, accuracy,
          location_type, address, event_time, ingest_source, payload_json
        ) VALUES ${placeholders}`,
        values,
      );
    }
  ...
  async publishKafkaEventsBatch(eventPayloads) {
    ...
    const topic = process.env.IOT_GEO_KAFKA_TOPIC || 'iot.geo.location.raw';
    ...
    try {
      ...
      console.error('[GeoLocation] Kafka batch publish failed:', error.message);
      return false;
    }
  }

  async ingestGeoEvent(rawPayload, source = 'http', meta = {}) {
    const batchResult = await this.ingestGeoBatch([rawPayload], source, meta);
    if (batchResult.success !== 1 || !batchResult.results[0]?.success) {
      throw new Error(batchResult.results[0]?.message || '地理定位数据接收失败');
    }
    return batchResult.results[0].data;
  }

  async ingestGeoBatch(rawEvents = [], source = 'http_batch', meta = {}) {
    await this.initGeoPipeline();
    ...
    normalizedEvents.forEach(payload => {
      ...
      const hasGeoMetric =
        hasMeaningfulMetricValue(payload.latitude) ||
        hasMeaningfulMetricValue(payload.longitude);

      if (!hasGeoMetric) {
        results.push({ success: false, message: '地理位置坐标不能为空（latitude/longitude）' });
        return;
      }
      ...
      insertRows.push([
        ...
        payload.device_id,
        assetCode,
        this.parseFloatSafe(payload.latitude),
        this.parseFloatSafe(payload.longitude),
        this.parseFloatSafe(payload.altitude),
        this.parseFloatSafe(payload.speed),
        this.parseFloatSafe(payload.heading),
  ...
  async publishKafkaEvent(eventPayload) {
    ...
    try {
      ...
      console.error('[GeoLocation] Kafka publish failed:', error.message);
      return false;
    }
  }
  ...
  async getLatestByDevice(deviceId, tenantId) {
    const [rows] = await db.execute(
      `SELECT * FROM iot_geo_location_ts
  ...
  getGeoPipelineHealth() {
    return {
      mqtt_enabled: process.env.IOT_GEO_MQTT_ENABLED === 'true',
      mqtt_connected: !!this.mqttClient?.connected,
      kafka_enabled: process.env.IOT_GEO_KAFKA_ENABLED === 'true',
      kafka_connected: !!this.kafkaProducer,
      tsdb: process.env.IOT_GEO_TSDB || 'mysql_ts_table',
  ...
  getGeoPipelineDocs() {
    return {
      ...
      http_ingest_path: '/api/iot-geo-location/ingest',
      http_ingest_batch_path: '/api/iot-geo-location/ingest/batch',
      auth: {
      ...
      mqtt: {
        enabled: process.env.IOT_GEO_MQTT_ENABLED === 'true',
        topic: process.env.IOT_GEO_MQTT_TOPIC || 'iot/geo/+/up',
      },
      kafka: {
        enabled: process.env.IOT_GEO_KAFKA_ENABLED === 'true',
        topic: process.env.IOT_GEO_KAFKA_TOPIC || 'iot.geo.location.raw',
      },
      payload_examples: {
        flat: {
          device_id: 'GPS001',
