...
class ZoneLocationPipelineService {
  ...
  async ingestBatch(rawEvents = [], source = 'http_batch', meta = {}) {
    ...
    for (const payload of normalizedEvents) {
      ...
      if (payload.location_code) {
        try {
          const locationUpdateResult = await this.handleBeaconLocationReport(
            payload.device_id,
  ...
  async handleBeaconLocationReport(deviceId, locationCode, options = {}) {
    const { expectedTenantId } = options;

    if (!deviceId || !locationCode) {
      return null;
    }

    const [existingLocations] = await db.execute(
      `SELECT id, tenant_id, asset_code FROM asset_locations
       WHERE device_id = ? AND location_code = ? AND is_active = 1
       LIMIT 1`,
      [deviceId, locationCode],
    );

    if (Array.isArray(existingLocations) && existingLocations.length > 0) {
      const location = existingLocations[0];
      if (expectedTenantId !== null && Number(location.tenant_id) !== expectedTenantId) {
        throw new Error('信标位置归属企业与当前租户不符');
      }
      await db.execute(
      ...
      return { asset_code: location.asset_code, location_id: location.id };
    }

    const [devices] = await db.execute(
      'SELECT tenant_id FROM iot_devices WHERE device_id = ? LIMIT 1',
      [deviceId],
    );
    ...
    if (expectedTenantId !== null && Number(tenantId) !== expectedTenantId) {
      throw new Error('设备租户与令牌所属企业不一致');
    }

    const [assetLocations] = await db.execute(
      `SELECT asset_code FROM asset_locations
       WHERE device_id = ? AND is_active = 1
       ORDER BY last_update_time DESC LIMIT 1`,
      [deviceId],
    );

    const assetCode = Array.isArray(assetLocations) && assetLocations.length > 0
      ? assetLocations[0].asset_code
      : null;

    await db.execute(
      `INSERT INTO asset_locations
       (tenant_id, device_id, location_code, asset_code, is_active, last_update_time, created_at, updated_at)
       VALUES (?, ?, ?, ?, 1, NOW(), NOW(), NOW())`,
      [tenantId, deviceId, locationCode, assetCode],
    );

    return { asset_code: assetCode, location_code: locationCode };
  }
