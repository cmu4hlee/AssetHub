...
const Login = () => {
  ...
  useEffect(() => {
    (async () => {
      const token = crypto.getItem('token') || await crypto.getItemAsync('token');
      const user = crypto.getItem('user') || await crypto.getItemAsync('user');
      if (token && user) {
        if (user?.tenant_id) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.enterprises?.length > 0) {
          ...
          message.error('您没有可访问的企业，请联系管理员分配企业权限');
  ...
  const onCodeLogin = async (values) => {
    try {
      setLoading(true);
      const result = await userAPI.loginWithCode(values.phone, values.code);
      if (result.success) {
        await crypto.setItemAsync('token', result.data.token);
        await crypto.setItemAsync('user', result.data.user);
