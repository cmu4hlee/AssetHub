...
| 模块名称 | 主要功能 |
|---------|---------|
| 基础资产管理 | 资产分类、资产信息、资产标签 |
| 折旧管理 | 折旧测算、部门/类型汇总、月度趋势、报表导出 |
| 资产盘点 | 盘点任务、盘点明细、差异处理 |
| 资产调配 | 调配申请、审批流程、位置更新 |
| 闲置发布 | 闲置资产发布、需求匹配 |
| 验收管理 | 验收申请、文件管理、电子签名 |
...
**模块拆分说明（2026-02-13）：**
- 折旧能力已从“基础资产管理”中拆分为独立模块“折旧管理”。
- 数据输入仍来源于`assets`表（如`purchase_date`、`purchase_price`、`depreciation_method`、`depreciation_years`等字段）。
- 折旧模块通过独立API提供测算、汇总、趋势和导出能力，便于按租户独立启停和权限控制。
...
```
depreciation-management (折旧管理逻辑模块)
└── assets (折旧输入字段)
    ├── purchase_date
    ├── purchase_price
    ├── depreciation_method
